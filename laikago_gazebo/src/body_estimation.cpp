...
void BodyPoseEstimator::update() {
    ...
    //todo: call kinematics class instead.
    Eigen::Quaternionf imu_quat(lowState.imu.quaternion[0],
    ...
    for (int i = 0; i < 4; i++) {
        ...
        dp_rel << highState.footSpeed2Body[i].x, highState.footSpeed2Body[i].y, highState.footSpeed2Body[i].z;
        ...
        rindex2 = 12 + i1;
        rindex3 = 24 + i;
        float phase = fminf(fmaxf((lowState.footForce[i] - low_) / (high_ - low_), 0), 1);
    ...
    highState.sidePosition.y = _xhat[1];
    highState.bodyHeight = _xhat[2];
    highState.forwardSpeed = _xhat[3];
